...
func TestDamageCreateResponse(t *testing.T) {
	t.Run("FromDbDamage", func(t *testing.T) {
		mockDamageModel := BuildTestDamage("1")

		resp := models.DamageCreateResponse{}
		resp.FromDbDamage(mockDamageModel, nil)
	...
	t.Run("FromDbDamageWithError", func(t *testing.T) {
		mockDamageModel := BuildTestDamage("1")

		resp := models.DamageCreateResponse{}
		mockError := errors.New("test error")
		resp.FromDbDamage(mockDamageModel, mockError)

		assert.Equal(t, mockDamageModel.ID, resp.ID)
		assert.Equal(t, mockError.Error(), resp.Error)
	})

	t.Run("DbDamageToCreateResponse", func(t *testing.T) {
		mockDamageModel := BuildTestDamage("1")

		resp := models.DbDamageToCreateResponse(mockDamageModel, nil)
