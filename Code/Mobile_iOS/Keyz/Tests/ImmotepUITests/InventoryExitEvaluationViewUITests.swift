//
//  InventoryExitEvaluationViewUITests.swift
//  Immotep
//
...
final class InventoryExitEvaluationViewUITests: XCTestCase {
    let app = XCUIApplication()

    override func setUpWithError() throws {
        continueAfterFailure = false
        app.launchArguments = ["-skipLogin", "-inventoryExitEvaluationView"]
        print("XCUITest - Launch arguments set to: \(app.launchArguments)")
        app.launch()
    }

    func testExitEvaluationViewElements() throws {
        let topBarTitle = app.staticTexts["Inventory Exit"]
        XCTAssertTrue(topBarTitle.exists, "The 'Inventory Exit' title should be visible")
